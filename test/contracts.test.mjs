...
test('package exposes one standard DSH bundle', async () => {
  ...
  assert.equal(pkg.name, 'dsh-token-monitor'); assert.equal(pkg.version, '1.3.0'); assert.equal(pkg.main, './src/index.mjs')
...
test('bundle adds only its own entry and client registers the settings section', async () => {
  ...
  assert.match(patch, /id: dsh-token-monitor/); assert.doesNotMatch(patch, /remove:|disable:|replace:/); assert.match(client, /window\.__ModuleLoader__\.load/); assert.match(client, /settings\.section/); assert.doesNotMatch(client, /settings\.plugins\.tab/); assert.match(client, /projectionValues\?\.tokenMonitorUsage/); assert.match(host, /sessionProjections\.register/); assert.doesNotMatch(host, /\breadFile\s*\(|\bwriteFile\s*\(|\bexec\s*\(|\bspawn\s*\(|\bfetch\s*\(/)
})
test('pricing and account-derived estimates are absent', async () => {
  const client = await readFile(new URL('src/client.js', root), 'utf8')
  assert.doesNotMatch(client, /estimatedCost|预算|每百万|localStorage|账号级推算[^。]*作为数据源/)
})
